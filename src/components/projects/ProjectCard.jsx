...
export default function ProjectCard({
    ...
    const projectAuthor = (users && project?.authorId && users[project.authorId]) || {};
    ...
    return (
        <article className="project-card" onClick={() => handleProjectClick(project.id)} style={{
            ...
            {/* Acciones de Docente y Administrador (Control Unificado) */}
            ...
            {/* Edición (Solo Autor para no solapar con acciones de profe/admin) */}
            {(isAuthor && userRole !== 'teacher' && userRole !== 'admin') && (
                <button
            ...
            <div className="card-body">
                ...
                <p className="card-description">{project.description || project.problemSolved}</p>
                ...
                <div className="author-info" onClick={(e) => { e.stopPropagation(); setViewedUserId(project.authorId); setActiveView('profile'); }} style={{ cursor: 'pointer' }}>
                    ...
                    <div className="author-details">
                        <span className="author-name">
                            {projectAuthor.firstName ? `${projectAuthor.firstName} ${projectAuthor.lastName || ''}` : (projectAuthor.name || 'Cargando...')}
                        </span>
                        {projectAuthor.program && <small className="author-program">{projectAuthor.program}</small>}
                    </div>
                </div>
                <div style={{ display: 'flex', width: '100%', alignItems: 'center', justifyContent: 'space-between', color: '#64748b', fontSize: '0.9rem' }}>
                    ...
                    <span className="card-date" style={{ textTransform: 'capitalize', fontWeight: '600' }}>{project.status}</span>
                </div>
            </div>
        </article>
    );
